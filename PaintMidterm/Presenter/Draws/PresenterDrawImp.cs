...
namespace PaintMidterm.Presenter.Draws
{
    class PresenterDrawImp : IPresenterDraw
    {
        ...
        public void handleClickToSelect(Point p)
        {
            ...
            {
                if (!(dataManager.shapeList[i] is Model.MPen))
                ...
                else if (dataManager.shapeList[i].isHit(p))
                {
                    ...
                    if (dataManager.shapeList[i] is Model.MPen)
                    {
                        if (((Model.MPen)dataManager.shapeList[i]).isEraser)
                        {
        ...
        private void handleClickToDraw(Point p)
        {
            ...
            {
                dataManager.addEntity(new Model.MRectangle
                {
                    pointHead = p,
                    pointTail = p,
                    contourWidth = dataManager.lineSize,
        ...
        public void onClickMouseMove(Point p)
        {
            ...
            {
                if (!(dataManager.shapeToMove is GroupShape) && !(dataManager.shapeToMove is Model.MPen))
                {
            ...
            else if (dataManager.currentShape.Equals(CurrentShapeStatus.Void))
            {
                ...
                else
                {
                    ...
                    //Kiếm tra xem trong danh sách tồn tại hình nào chứa điểm p không
            ...
            {
                Model.MPen pen = dataManager.shapeList[dataManager.shapeList.Count - 1] as Model.MPen;
                pen.points.Add(p);
        ...
        {
            if (shape is Model.MPen)
            {
                Model.MPen pen = shape as Model.MPen;
        ...
        public void onClickMouseUp()
        {
            ...
            else if (dataManager.isMovingMouse)
            {
                ...
                for (int i = 0; i < dataManager.shapeList.Count; ++i)
                {
                    ...
                    if (dataManager.shapeList[i] is Model.MPen)
                    {
                        Model.MPen pen = dataManager.shapeList[i] as Model.MPen;
                        if (pen.isEraser)
                            dataManager.shapeList[i].isSelected = false;
                    }
                }
        ...
        public void getDrawing(Graphics g)
        {
            ...
            if (dataManager.isMovingMouse)
            {
                using (System.Drawing.Pen pen = new System.Drawing.Pen(Color.DarkBlue, 1)
                {
        ...
        private void drawRegionForShape(Shape shape, Graphics g)
        {
            ...
            else if (shape is Model.MPen)
            {
                if (!((Model.MPen)shape).isEraser)
                {
                    using (System.Drawing.Pen pen = new System.Drawing.Pen(Color.DarkBlue, 1)
                    {
                        DashPattern = new float[] { 3, 3, 3, 3 },
